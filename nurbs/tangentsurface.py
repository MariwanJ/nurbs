...
def createShape(obj, force=False):
    ...
    if 1:
        ...
        pts2 = np.concatenate(
            [[l, pts[0], pts[0], pts[0], l2], pts[1:-1], [r, pts[-1], pts[-1], pts[-1], r2]])
...
#        obj.addProperty("App::PropertyFloat","P","Base").P=1.8
        ...
        obj.addProperty("App::PropertyEnumeration", "displayShape", "Base").displayShape = [
            "Seam", "OutSeam", "InSeam", "Curve", "Tangent1", "Tangent2"]
        obj.linear = True
    ...
    def execute(proxy, obj):
        ...
        if obj.source != None:
            ...
            spols = np.array([
                poles[0]+(poles[0]-poles[1])*obj.factorB*0.1 +
                (poles[1]-poles[2])*obj.factorC*0.1,
                poles[0]+(poles[0]-poles[1])*obj.factorA*0.1,
                poles[0],
                poles[0]+(poles[1]-poles[0])*obj.factorA*0.1,
                poles[0]+(poles[1]-poles[0])*obj.factorB*0.1 +
                (poles[2]-poles[1])*obj.factorC*0.1
            ])
            ...
            if obj.fillCorner:
                ...
                spolsA = np.concatenate([
                    ...
                    [poles[-1],
                     ...
                     poles[-1]-(poles[-2]-poles[-1])*obj.factorB * \
                     0.1-(poles[-3]-poles[-2])*obj.factorC*0.1
                     ]
                ])
            ...
            if obj.endPlane != None:
                ...
                V = pl.Placement.Rotation.multVec(
                    App.Vector(0, 0, 1)).normalize()
                ...
                if len(ends) >= 2:
                    ...
                    bs = machFlaeche(
                        spols2, ku=None, closed=closed, bs=sf, swap=obj.sourceSwap)
...
class Nurbs_TangentSurfaceAnotherMain:
    ...
    def GetResources(self):
        ...
        return {'Pixmap':  NURBSinit.ICONS_PATH + 'alpha.svg',
                'MenuText': QT_TRANSLATE_NOOP("Nurbs", "Nurbs_TangentSurfaceAnotherMain"),
...
class Nurbs_TangentMainFunction:
    ...
    def GetResources(self):
        ...
        return {'Pixmap':  NURBSinit.ICONS_PATH + 'alpha.svg',
                'MenuText': QT_TRANSLATE_NOOP("Nurbs", "Nurbs_TangentMainFunction"),
                'ToolTip': QT_TRANSLATE_NOOP("Nurbs", _tooltip)}
...
class NurbsTangentSurfaceRUNSEAM:
    ...
    def GetResources(self):
        ...
        return {'Pixmap':  NURBSinit.ICONS_PATH + 'alpha.svg',
                'MenuText': QT_TRANSLATE_NOOP("Nurbs", "NurbsTangentSurfaceRUNSEAM"),
                'ToolTip': QT_TRANSLATE_NOOP("Nurbs", _tooltip)}
...
class NurbsTangentSurfaceRUN:
    ...
    def GetResources(self):
        ...
        return {'Pixmap':  NURBSinit.ICONS_PATH + 'alpha.svg',
                'MenuText': QT_TRANSLATE_NOOP("Nurbs", "NurbsTangentSurfaceRUN"),
                'ToolTip': QT_TRANSLATE_NOOP("Nurbs", _tooltip)}
...
#        mu=[4]+[1]*(NbUPoles-4)+[4]
    ...
    if wew != None:
        bs2.buildFromPolesMultsKnots(
            ps, mv, mu, kv, ku,  False, closed, vdegree, degree, wew)
    else:
        bs2.buildFromPolesMultsKnots(
            ps, mv, mu, kv, ku,  False, closed, vdegree, degree)
    return bs2

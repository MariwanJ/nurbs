...
class gridNode(coin.SoSeparator):
    ...
    def vector1dir(self, vec):
        ...
        self.normal = self._vector1.cross(self._vector2)
        self.calc.expression.set1Value(2, "tA=vec3f(%f,%f,%f)" % (
            self.normal.getValue()[0], self.normal.getValue()[1], self.normal.getValue()[2]))
        self.buildGrid()
    ...
    def updateTransformedNormal(self):
        ...
        self.calc.expression.set1Value(2, "tA=vec3f(%f,%f,%f)" % (self.transformedNormal.getValue()[
                                       0], self.transformedNormal.getValue()[1], self.transformedNormal.getValue()[2]))
        return()
...
class gridObject:
    ...
    def onChanged(self, fp, prop):
        if prop == 'Placement':
            ...
            fp.ViewObject.Proxy.trans.translation = coin.SbVec3f(
                tr.x, tr.y, tr.z)
            fp.ViewObject.Proxy.trans.rotation = coin.SbRotation(
                ro[0], ro[1], ro[2], ro[3])
...
class gridVP:
    ...
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyDistance",  "Subdivision",
                        "Size",   "Size of subdivisions").Subdivision = '10mm'
        obj.addProperty("App::PropertyFloat",     "XY_Attenuation",
                        "View",   "XY plane attenuation").XY_Attenuation = 2.0
        obj.addProperty("App::PropertyFloat",     "XZ_Attenuation",
                        "View",   "XZ plane attenuation").XZ_Attenuation = 50.0
        obj.addProperty("App::PropertyFloat",     "YZ_Attenuation",
                        "View",   "YZ plane attenuation").YZ_Attenuation = 50.0
        obj.addProperty("App::PropertyFloat",     "XY_Visibility",
                        "View",   "XY plane max visibility").XY_Visibility = 1.0
        obj.addProperty("App::PropertyFloat",     "XZ_Visibility",
                        "View",   "XZ plane max visibility").XZ_Visibility = 0.5
        obj.addProperty("App::PropertyFloat",     "YZ_Visibility",
                        "View",   "YZ plane max visibility").YZ_Visibility = 0.5
        obj.addProperty("App::PropertyColor",     "GridColor",
...
class Nurbs_ParametricGridModifiable:
    ...
    def GetResources(self):
        ...
        return {'Pixmap':  NURBSinit.ICONS_PATH + 'drawing.svg',
                'MenuText': QT_TRANSLATE_NOOP("Nurbs", "Nurbs_ParametricGridModifiable"),
